...
const ProcessingAnimation = ({
    ...
    if (!isYouTube && videoRef.current) {
      if (isSyncedPlaying) {
        ...
        if (isComplete) {
          ...
          videoRef.current
            .play()
            .catch((e) => console.log('Ambient play prevented', e));
        }
      }
    }
  ...
  return (
    <div className={containerClasses}>
      {/* Video Layer */}
      <div
        className={`absolute inset-0 transition-all duration-700 ${getVideoOpacityClass()}`}
      >
        {isYouTube && videoSrc ? (
          <iframe
            ref={iframeRef}
      ...
      {!isSyncedPlaying && (
        <div
          className={`absolute top-4 left-4 z-30 flex items-center gap-2 px-3 py-1.5 backdrop-blur-md rounded-lg border text-xs font-mono font-bold uppercase transition-all duration-500 ${isComplete ? 'bg-green-500/10 border-green-500/20 text-green-400' : 'bg-black/60 border-primary/30 text-primary animate-pulse'}`}
        >
          {isComplete ? (
            <>
