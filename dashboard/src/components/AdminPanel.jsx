...
export default function AdminPanel() {
  ...
  const loadAll = useCallback(async () => {
    try {
      const [u, n, y, f] = await Promise.all([
  ...
  const tabs = ['Users', 'Niches', 'YT Channels', 'FB Pages'];
          ...
          <td className="py-3 px-4">
            <div className="flex gap-2">
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-violet-500/10 hover:bg-violet-500/20 text-violet-300 text-xs font-medium transition-colors"
              >
              ...
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-300 text-xs font-medium transition-colors"
              >
                <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polygon points="9.75 15.02 15.5 11.75 9.75 8.48 9.75 15.02"/><path d="M21.54 4.15A1.98 1.98 0 0 0 20 3.5H4c-.83 0-1.54.6-1.54 1.4v12.2c0 .8.71 1.4 1.54 1.4h16c.83 0 1.54-.6 1.54-1.4V4.55c0-.26-.1-.5-.27-.7l-.73-.7z"/></svg>
                YT
              </button>
            </div>
          </td>
